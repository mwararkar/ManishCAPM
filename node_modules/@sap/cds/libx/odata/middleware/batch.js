  ...
  for (const response of responses) {
    if (response.status === 'fail' && typeof response.body === 'object') {
      if (response._next_chain) {
        // Object.create to avoid toJSON already defined error
        const res = response._next_chain(Object.create(response.body.error))
...
const _processBatch = async (srv, router, req, res, next, body, ct, boundary) => {
  if (!error_mws) {
    // filter: error middleware has 4 params
    const adapter = router.stack.filter(l => l.handle.length === 4).map(l => l.handle)
    const middlewares = cds.middlewares.after.filter(mw => mw.length === 4)
    error_mws = [...adapter, ...middlewares]
  }
  ...
  router.stack = router.stack.filter(l => l.handle.length < 4)
  ...
  try {
    ...
    let { requestsById, atomicityGroups } = _deconstructAndValidateBatchBody(body)
...
module.exports = adapter => {
  ...
  return function odata_batch(req, res, next) {
    ...
    if (req.headers['content-type'].includes('application/json')) {
      return _processBatch(service, router, req, res, next, req.body, 'JSON')
    }
